...
def test_compute_quotas_sums_to_k_with_largest_remainder():
    # compute_quotas is now a static method on AssignmentService
    weights = {"a": 0.5, "b": 0.3, "c": 0.2}
    q = AssignmentService.compute_quotas(weights, 10)
...
def test_compute_quotas_handles_empty_or_zero_weights():
    # compute_quotas is now a static method on AssignmentService
    assert AssignmentService.compute_quotas({}, 5) == {}
    q = AssignmentService.compute_quotas({"a": 0.0, "b": -1.0}, 7)
    assert q == {"a": 0, "b": 0}
