...
describe("TagChip", () => {
	it("renders manual tag with violet styling", () => {
		...
		// Use getAllByText and select the first (non-tooltip) element
		const elements = screen.getAllByText("manual:tag");
		const chip = elements[0].closest("span");
	...
	it("renders computed tag with slate styling and lock icon", () => {
		...
		// Use getAllByText and select the first (non-tooltip) element
		const elements = screen.getAllByText("computed:tag");
		const chip = elements[0].closest("span");
